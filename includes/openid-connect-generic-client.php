...
class OpenID_Connect_Generic_Client {
	...
	function validate_token_response( $token_response ){
		...
		if ( ! isset( $token_response['id_token'] ) )  {
			return new WP_Error( 'invalid-token-response', 'The id is not set in the token', $token_response );
		} 
		else if (! isset( $token_response['token_type'] ) ) {
			return new WP_Error( 'invalid-token-response', 'The token type is not set', $token_response );
		}
		else if (strcasecmp( $token_response['token_type'], 'Bearer' )) {
